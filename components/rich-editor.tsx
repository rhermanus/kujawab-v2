...
// ─── Main reusable editor ────────────────────────────────────────────

interface RichEditorProps {
  onSubmit: (html: string) => Promise<{ success: boolean; error?: string }>;
  submitLabel: string;
  placeholder?: string;
  minHeight?: string;
...
export default function RichEditor({
  ...
  const handleSubmit = async () => {
    ...
    if (html === "<p></p>" || html.trim().length < minContentLength) {
      setError("Konten terlalu pendek.");
      return;
    }
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        {onCancel && (
          <button
            type="button"
            onClick={onCancel}
            className="px-4 py-2 rounded-lg border hover:bg-zinc-100 dark:hover:bg-zinc-800 text-sm"
          >
            Batal
          </button>
        )}
